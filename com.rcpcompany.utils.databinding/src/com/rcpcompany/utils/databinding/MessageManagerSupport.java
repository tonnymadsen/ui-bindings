...
public final class MessageManagerSupport {
	 ...
	 */
	public static MessageManagerSupport create(IMessageManager mm, DataBindingContext context) {
				...
				public void handleValueChange(ValueChangeEvent event) {
					if (myMessageManager.isAutoUpdate()) {
						...
						PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
							@Override
							public void run() {
								myMessageManager.setAutoUpdate(true);
							}
						});
					}
					...
					// "Provider: " +
					// p + " - event: " + event);
					final List<IObservableValue> targets = p.getTargets();

					/*
					 * See if the status message is associated with a specific control...
					 */
					Control control = null;
					if (targets.get(0) instanceof ISWTObservable) {
						final Widget widget = ((ISWTObservable) (targets.get(0))).getWidget();
					...
					if (status.getSeverity() != IStatus.OK) {
						final String messageKey = String.valueOf(status.hashCode());
						if (control == null) {
							myMessageManager.removeMessage(messageKey);
						} else {
							myMessageManager.removeMessage(messageKey, control);
						}
					}
					...
					if (status.getSeverity() != IStatus.OK) {
						...
						final String messageKey = String.valueOf(status.hashCode());
						if (control == null) {
							myMessageManager.addMessage(messageKey, status.getMessage(), null, errorType);
						} else {
							myMessageManager.addMessage(messageKey, status.getMessage(), null, errorType, control);
						}
					}
				}
			});
