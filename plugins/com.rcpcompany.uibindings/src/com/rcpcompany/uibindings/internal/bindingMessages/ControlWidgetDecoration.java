...
public class ControlWidgetDecoration implements IWidgetDecoration, IDisposable {
	 ...
	 * alignment of the decoration relative to the control. Decorations always appear on either the left or right side
	 * of the control, never above or below it. For example, a decoration appearing on the left side of the field, at
	 * the top, is specified as SWT.LEFT | SWT.TOP. If no position style bits are specified, the control decoration will
	 * be positioned to the left and center of the control (<code>SWT.LEFT | SWT.CENTER</code>).
	 * </p>
	 * 
	 ...
	 *            to this composite, but it may be rendered on a child of the composite. The decoration will not be
	 *            visible if the specified composite or its child composites are not visible in the space relative to
	 *            the control, where the decoration is to be rendered. If this value is <code>null</code>, then the
	 *            decoration will be rendered on whichever composite (or composites) are located in the specified
	 *            position.
	 */
